...
def get_oidc_token_url() -> str:
    """Return provider token URL with Auth0 compatibility."""
...
def get_dynamic_backend_url(request: Request) -> str:
    ...
    Priority: PYTHON_BACKEND_URL (env) > mgx-external-domain > x-forwarded-host > host > settings.backend_url

    Auth0 allowlists often use ``http://localhost:8000/...`` only; we normalize ``127.0.0.1`` → ``localhost``.
    """
    forced = (os.environ.get("PYTHON_BACKEND_URL") or "").strip()
    if forced:
        return _local_patch(forced.rstrip("/"))
    ...
    # Local development on uvicorn usually runs over HTTP (no TLS cert),
    # so force http:// for localhost-style hosts to avoid HTTPS callback failures.
    if effective_host.startswith("localhost") or effective_host.startswith("127.0.0.1"):
        scheme = "http"

    dynamic_url = _local_patch(f"{scheme}://{effective_host}")
    # Match Auth0 "Allowed Callback URLs" that list localhost but not 127.0.0.1
    if "127.0.0.1" in dynamic_url:
        dynamic_url = dynamic_url.replace("127.0.0.1", "localhost")
    logger.debug(
...
def get_dynamic_frontend_url(request: Request) -> str:
    """Resolve frontend base URL for post-login redirects."""
    configured_frontend = getattr(settings, "frontend_url", "")
...
async def login(request: Request, db: AsyncSession = Depends(get_db)):
    """Start OIDC login flow with PKCE. Returns JSON for the SPA (same shape as /logout)."""
    if not (settings.oidc_issuer_url and settings.oidc_client_id and settings.oidc_client_secret):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="OIDC is not configured. Set OIDC_ISSUER_URL, OIDC_CLIENT_ID, and OIDC_CLIENT_SECRET in the backend .env",
        )
    ...
    # Build redirect_uri dynamically from request (must exactly match an entry in Auth0 Allowed Callback URLs)
    ...
    return LoginStartResponse(redirect_url=auth_url)
