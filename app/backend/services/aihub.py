...
class AIHubService:
    ...
    def __init__(self):
        ...
        if settings.app_ai_base_url and settings.app_ai_key:
            base = settings.app_ai_base_url.rstrip("/")
            extra_headers: dict[str, str] = {}
            # OpenRouter recommends these for attribution (optional but polite)
            if "openrouter.ai" in base.lower():
                extra_headers["HTTP-Referer"] = getattr(
                    settings, "frontend_url", "http://localhost:3000"
                ).rstrip("/")
                extra_headers["X-Title"] = getattr(settings, "app_name", "PersonaRewire")
            client_kw: dict = {"api_key": settings.app_ai_key, "base_url": base}
            if extra_headers:
