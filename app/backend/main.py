...
async def lifespan(app: FastAPI):
    ...
    # Mock data seeding is disabled by default. Set SEED_MOCK_DATA=1 to re-enable.
    if os.environ.get("SEED_MOCK_DATA") == "1":
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    """
    # HTTPException subclasses Exception; do not re-raise from here (that often becomes 500).
...
def public_runtime_config():
    """Frontend bootstrapping (e.g. Vite fetch('/api/config'))."""
