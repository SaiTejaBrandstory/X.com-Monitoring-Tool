...
async def validate_id_token(id_token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Some providers (including Auth0 in certain setups) may return issuer
        # with/without trailing slash, so validate against both normalized forms.
        issuer_candidates = []
        base_issuer = settings.oidc_issuer_url.strip()
        if base_issuer:
            issuer_candidates.extend([base_issuer, base_issuer.rstrip("/"), f"{base_issuer.rstrip('/')}/"])
        # Preserve order while deduplicating
        issuer_candidates = list(dict.fromkeys([i for i in issuer_candidates if i]))

        last_claims_error: Optional[Exception] = None
        ...
        # If all issuer variants failed with claims errors, report detailed reason.
        if last_claims_error:
            logger.error(f"JWT validation failed: Claims validation error: {last_claims_error}")
            if "iss" in str(last_claims_error).lower() or "issuer" in str(last_claims_error).lower():
                raise IDTokenValidationError("Token issuer validation failed", "invalid_issuer")
            elif "aud" in str(last_claims_error).lower() or "audience" in str(last_claims_error).lower():
